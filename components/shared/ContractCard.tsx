            ...
            <AlertDialog open={isModalOpen} onOpenChange={setIsModalOpen}>
              <AlertDialogTrigger asChild>
                <Button size="sm" className={cn(buttonClass)} onClick={handleTriggerClick} disabled={isProcessing}>
                    {isProcessing && !pdfUrl && userType === 'doctor' ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Edit className="mr-2 h-4 w-4" />} 
                    Rever e Assinar
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent className="max-w-4xl h-[90vh] flex flex-col">
                  ...
                  <div className="flex-grow my-4 border rounded-md overflow-hidden bg-gray-200">
                      {isProcessing && !pdfUrl ? <LoadingState message="A gerar documento..."/> : 
                       ...
                       (userType === 'hospital' && !contract.contractPdfUrl) ? <ErrorState message="Documento PDF ainda não foi gerado pelo médico."/> :
