...
export const ContractCard: React.FC<ContractCardProps> = ({ contract, onSign, userType }) => {
  ...
  const handleTriggerClick = async () => {
    if (userType === 'hospital' && !pdfUrl) {
      if(contract.contractPdfUrl) {
        setPdfUrl(contract.contractPdfUrl);
      } else {
        toast({ title: "Documento não encontrado", description: "O PDF do contrato ainda não foi gerado pelo médico.", variant: "destructive" });
        return;
      }
    }
    
    if (userType === 'doctor' && !pdfUrl) {
  ...
  const displayShiftDates = Array.isArray(contract.shiftDates) ? contract.shiftDates.map((ts) => ts.toDate().toLocaleDateString('pt-BR', {day: '2-digit', month: 'short'})).join(', ') : "Datas não disponíveis";
        ...
        <CardFooter className="flex justify-end items-center pt-3 pb-3">
            <AlertDialog open={isModalOpen} onOpenChange={setIsModalOpen}>
              <AlertDialogTrigger asChild>
                <Button size="sm" className={cn(buttonClass)} onClick={handleTriggerClick} disabled={isProcessing && !pdfUrl}>
              ...
              <AlertDialogContent className="max-w-4xl h-[90vh] flex flex-col">
                  ...
                  <div className="flex-grow my-2 border rounded-md overflow-hidden bg-gray-200">
                      {isProcessing ? <LoadingState message="A processar..."/> : 
                       pdfUrl ? <iframe src={pdfUrl} className="w-full h-full" title="Contrato PDF"/> : 
                       <ErrorState message="Não foi possível carregar o documento." onRetry={handleTriggerClick}/>
                      }
                  </div>
                  <AlertDialogFooter>
