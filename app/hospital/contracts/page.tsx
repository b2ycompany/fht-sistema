...
import { Loader2, ClipboardList, AlertTriangle, UserCheck, CalendarDays, Clock, DollarSign, Briefcase, RotateCcw, Edit } from 'lucide-react';
    ...
    <>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>Revisão e Assinatura do Contrato</DialogTitle>
          </DialogHeader>
          <DialogDescription className="text-sm text-muted-foreground mb-2">
            Reveja o contrato antes de assinar. Sua assinatura será registrada oficialmente.
          </DialogDescription>
          <div className="flex-grow my-2 border rounded bg-gray-100 overflow-hidden">
            {contract.contractPdfUrl ? (
              ...
              <div className="flex h-full items-center justify-center p-4 text-red-600">
                Documento não encontrado. O médico pode não ter gerado o PDF.
              </div>
            )}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsModalOpen(false)} disabled={isSigning}>
              Voltar
            </Button>
            <Button
              onClick={handleConfirmSignature}
              disabled={isSigning || !contract.contractPdfUrl}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="pending">
            Contratos Pendentes <Badge variant={pendingContracts.length > 0 ? "destructive" : "secondary"} className="ml-2">{isLoading ? '...' : pendingContracts.length}</Badge>
          </TabsTrigger>
          <TabsTrigger value="active">
            Contratos Ativos <Badge variant="secondary" className="ml-2">{isLoading ? '...' : activeContracts.length}</Badge>
          </TabsTrigger>
        </TabsList>
