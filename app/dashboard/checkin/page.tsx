...
interface CameraModalProps { isOpen: boolean; onClose: () => void; onCapture: (dataUrl: string) => void; }
    ...
    useEffect(() => {
        ...
        const startCamera = async () => {
            try {
                ...
                } else { setError("Não foi possível aceder à câmera."); }
            }
        };
        startCamera();
        return () => {
...
const ReverseGeocodedLocation: React.FC<{ location: { latitude: number; longitude: number; } }> = ({ location }) => {
    ...
    useEffect(() => {
        ...
        const fetchAddress = async () => {
            try {
                ...
                setAddress((city && state) ? `${city}, ${state}` : "Nome do local não encontrado");
            } catch (error) {
    ...
    return ( <div className="text-xs text-gray-500 flex items-center"><Target size={13} className="mr-1.5" />Local registado: {address}</div> );
};
...
export default function DoctorCheckinPage() {
    ...
    const fetchActiveShifts = useCallback(async () => {
        ...
        } catch (err: any) { setError(err.message || "Falha ao carregar plantões."); }
    ...
    const handleStartTelemedicine = async (contractId: string) => {
        ...
        try {
            await createTelemedicineRoom(contractId);
            router.push(`/telemedicine/${contractId}`);
        } catch (error: any) {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                try {
                    ...
                    fetchActiveShifts();
                } catch (err: any) { toast({ title: "Erro ao fazer Check-out", description: err.message, variant: "destructive" }); }
                finally { setActionLoadingId(null); }
            },
            (geoError) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader><CardTitle>Seus Próximos Plantões</CardTitle><CardDescription>Realize o check-in ao iniciar e o check-out ao finalizar seus plantões aqui.</CardDescription></CardHeader>
                <CardContent>
            ...
            <div className="mt-6 p-4 border rounded-lg bg-amber-50 text-amber-800">
                <div className="flex items-start"><AlertTriangle className="h-5 w-5 mr-3 mt-0.5 text-amber-600" /><div><h3 className="font-semibold">Importante sobre Geolocalização e Câmera</h3><p className="text-xs mt-1">Para realizar o check-in, o seu navegador solicitará permissão para aceder à sua localização e câmera. Isto é necessário para validar a sua presença e identidade no local do plantão.</p></div></div>
            </div>
        </div>
    );
}
        ...
        <Card className={cn("shadow-sm transition-all", record.status === 'CHECKED_IN' && "border-green-500 ring-2 ring-green-500/50")}>
            <CardHeader>
                <div className="flex justify-between items-start">
                    ...
                    <Badge variant={statusBadgeInfo.variant} className={cn("capitalize", statusBadgeInfo.className)}>{record.status.replace(/_/g, ' ').toLowerCase()}</Badge>
                </div>
